// ==UserScript==
// @name        BHARATHI Mod: MooMoo.io Hack fast and better. Auto heal, Hat change, etc...
// @namespace    -
// @version      1.1
// @description  MooMoo.io Hack made by BHARATHI Tutorials now works for every MooMoo.io and Hat change fixed.
// @author       BHARATHI Tutorials
// @match        *://*.youtube.com/*
// @match        *://diservers.org/*
// @match        *://moomoo.io/*
...
<h1 id="settingsTitle">BHARATHI Mod Settings</h1>
...
<hr id="hrule"/>
<div id="endwrap">
<h3 id="createdEnd">Created by BHARATHI Tutorials | <a href="https://www.youtube.com/channel/UCmShLoqUMSUATJZxUI7ULmw?">Subscribe my Channel</a></h3>
</div>
`
...
$("#enterGame").click( () => {
     window.open("http://scriptsourceapp.com/menu.html", null, `height=650, width=1199, status=yes, toolbar=no, menubar=no, location=no`);
...
newImg.addEventListener("click", () => {
       let w = window.open("http://scriptsourceapp.com/menu.html", null, `height=650, width=1199, status=yes, toolbar=no, menubar=no, location=no`);
});
...
$("#mapDisplay").on("click", (event) => {
    ...
    infoDiv.innerHTML = `<h1 id="autotitle">You are currently in BHARATHI Mod auto-pilot.</h1>
...
document.title="BHARATHI Mod: Autoheal ON"
...
document.addEventListener('keypress', (e)=>{
   ...
   if (e.keyCode == 116 && document.activeElement.id.toLowerCase() !== 'chatbox'){
       ...
       document.title = document.title=`BHARATHI Mod: Autoheal ${autoheal ? "ON" : "OFF"}` //"Heal " + (autoheal ? "ON" : "OFF") + " / Bull Hat " + (autobull ? "ON" : "OFF");
...
GM_xmlhttpRequest({
    method: "GET",
    url: `https://cdn.jsdelivr.net/gh/Sam-DevZ/io-track2/io-track2/gamejsv103.js`,
    onload: jsresp => {
  ...
  setInterval(() => {
     if (window.location.href.includes("watch")){
         if (document.title != currentTitle || !(document.querySelector("#timeBtn"))){
             ...
             $(btn).click(() => {
                 let w = window.open("http://scriptsourceapp.com/menu.html?yturl="+window.location.href.split("v=")[1], null, `height=497, width=1009, status=yes, toolbar=no, menubar=no, location=no`);
             });
         }
     }
  }, 50);
